...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\mssaa\\OneDrive\\Desktop\\PSYCHOPY\\Intentional binding_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "beeb" ---
    sound_1 = sound.Sound(
        'A', 
        secs=0.3, 
        stereo=True, 
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('C:/Users/mssaa/Downloads/ib_cons - Sheet1.csv'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "beeb" ---
        # create an object to store info about Routine beeb
        beeb = data.Routine(
            name='beeb',
            components=[sound_1],
        )
        beeb.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        sound_1.setSound(freq, secs=0.3, hamming=True)
        sound_1.setVolume(1.0, log=False)
        sound_1.seek(0)
        # store start times for beeb
        beeb.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        beeb.tStart = globalClock.getTime(format='float')
        beeb.status = STARTED
        thisExp.addData('beeb.started', beeb.tStart)
        beeb.maxDuration = None
        # keep track of which components have finished
        beebComponents = beeb.components
        for thisComponent in beeb.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        beeb.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                beeb.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in beeb.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in beeb.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for beeb
        beeb.tStop = globalClock.getTime(format='float')
        beeb.tStopRefresh = tThisFlipGlobal
        thisExp.addData('beeb.stopped', beeb.tStop)
        sound_1.pause()  # ensure sound has stopped at end of Routine
        # the Routine "beeb" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
